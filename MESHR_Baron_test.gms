$title Rective ditillation - Exhaustive - only BARON scaling ON
...
*                X_D_ETBE(j)$(ord(j) = Ns)  = x.l['4',j];
*                RR = L.l('1')/D.l;
*                DISPLAY Ns, NFE, NFB, NR, X_ETOH, x_D_ETBE, RR, Qr.l, MESHR_Rigorous.modelStat;;
*                model_stat_conopt(j,jj,zz,xx,hh)$(CONDITION_2) = MESHR_Rigorous.modelStat;
*                if ((MESHR_Rigorous.modelStat = 1) or (MESHR_Rigorous.modelStat = 2),
*                    x_ETBE_conopt(j,jj,zz,xx,hh) $(CONDITION_2) =  X_D_ETBE(j);
*                    X_ETOH_copnopt(j,jj,zz,xx,hh) $(CONDITION_2) =  X_ETOH(j);
*                    profit_obj_conopt(j,jj,zz,xx,hh)$(CONDITION_2)  = obj.l;
*                    count_s_conopt = count_s_conopt+1;
*                    
*                    else
*                        x_ETBE_conopt(j,jj,zz,xx,hh) $(CONDITION_2) = -1;
*                        X_ETOH_copnopt(j,jj,zz,xx,hh) $(CONDITION_2) = -1;
*                        profit_obj_conopt(j,jj,zz,xx,hh)$(CONDITION_2)  = -1;
*                );
...
if ((MESHR_Rigorous.modelStat = 1) or (MESHR_Rigorous.modelStat = 2),
    ...
    FE_array(j,jj,zz,xx,hh)$(CONDITION_2) = FE.l*F_Factor;
    count_s = count_s+1;
        ...
        FE_array(j,jj,zz,xx,hh)$(CONDITION_2) = -1;
        count_f = count_f+1;
);
